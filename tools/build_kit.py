...
# Theme files only. Photos, slides and videos are content: they go into the
# admin panel through the migration pack (tools/admin_pack.py), never here.
COPY_DIRS = ["fonts", "brand"]
...
def main():
    # No content in the kit: everything a page shows comes from the admin
    # panel. A leftover content.php from older kits is removed.
    old = os.path.join(KIT, "resources", "redesign", "content.php")
    ...
    n3 = sync_assets()
    print(f"kit: icons {'updated' if n2 else 'unchanged'} · theme assets changed: {n3}")
