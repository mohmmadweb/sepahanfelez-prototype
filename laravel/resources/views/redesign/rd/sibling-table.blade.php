...
@foreach($others as $r)
<tr><td class="pt-name">{{ Rd::link(Rd::prodUrl($slug, $r['_slug']), Rd::cleanName($r['نام محصول'])) }}</td>@foreach($specs as $x)<td data-label="{{ $R::shortHead($x) }}">{{ Rd::cleanVal($r[$x] ?? '') }}</td>@endforeach<td class="pt-price" data-label="قیمت (ریال)"><b class="num">{{ Rd::fmt($r['_price']) }}</b></td></tr>
@endforeach
</tbody></table></div>
